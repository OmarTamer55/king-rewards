...
client.user.setGame(`Húntèrs SyStem | $help`,"http://twitch.tv/S-F")
  console.log('')
  console.log('')
  console.log('╔[═════════════════════════════════════════════════════════════════]╗')
...
client.on('message', message => {
    ...
    if(!message.channel.guild) return message.channel.send('**This Command Only For Servers !**')
            if (!message.member.hasPermission('MANAGE_ROLES')) return message.channel.send(`**${message.author.username} You Dont Have** ``MANAGE_ROLES`` **Premission**`);
                     ...
                     message.guild.createRole({ name: "Assi", color: "#ffffff", permissions: [] })
                     message.guild.createRole({ name: "Co-Leader", color: "#ffffff", permissions: [] })
                     message.guild.createRole({ name: "King", color: "#ffffff", permissions: [] })
                     message.guild.createRole({ name: "Qween", color: "#ffffff", permissions: [] })
                     message.guild.createRole({ name: "HighNess", color: "#ffffff", permissions: [] })
...
client.on("guildMemberAdd", member => {
        if(member.guild.id === "515908660975632385") {  // ايدي السيرفر
...
client.on("guildMemberRemove", member => {
  ...
  const channel = member.guild.channels.find('id', '516241723295399959'); 
if (!channel) return;
  channel.send(`**${member.user.tag} Left The Server  ** 😭 `) 
}});
client.on('guildMemberAdd', member=> {
    member.addRole(member.guild.roles.find("name","Húntèrs")); //اسم ا��رتبة
    });

client.on("ready", () => { // كود رينبو
  function lol() {
    client.guilds.get('515908660975632385').roles.find("name", "RainBow").setColor("RANDOM");
  };
...
var prefix = "$";
   ...
   if(!message.member.hasPermission('MANAGE_MESSAGES')) return message.reply(' **Room Has Been Closed**');
              message.channel.overwritePermissions(message.guild.id, {
            SEND_MESSAGES: false

              }).then(() => {
                  message.reply("**Room Has Been Closed:white_check_mark: **")
              });
                }
//FIRE BOT
   ...
   if(!message.member.hasPermission('MANAGE_MESSAGES')) return message.reply('**Room Has Been Opened**');
              message.channel.overwritePermissions(message.guild.id, {
            SEND_MESSAGES: true

              }).then(() => {
                  message.reply("**Room Has Been Opened:white_check_mark:**")
              });
    }
  });     
var prefix = "$";
...
client.on('voiceStateUpdate', (old, now) => {
  const channel = client.channels.get('518589034361258025');
...
client.on('message', function(message) {
    ...
    client.channels.get("516276800943554571").send({embed:iiMo});
    }
});
...
client.login('NTE4NzAxMzA2NDM3Njk3NTQ3.DuUoRg.Rx9wwU-IN9m9Otl3_Ts6235H6mo');
